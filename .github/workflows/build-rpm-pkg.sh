...
AVALANCHE_LIB_DIR=$RPM_BASE_DIR/usr/local/lib/metalgo
...
[ -z "$REL" ] && REL=0 
echo "Tag: $VER"
rpmbuild --bb --define "version $VER" --define "release $REL" --buildroot $RPM_BASE_DIR .github/workflows/yum/specfile/metalgo.spec
aws s3 cp ~/rpmbuild/RPMS/x86_64/metalgo-*.rpm s3://$BUCKET/linux/rpm/
